...
class PackageRuleTest : WordSpec() {
    ...
    private fun PackageRule.createLicenseRule(license: SpdxSingleLicenseExpression) =
        LicenseRule(
            name = "test",
        ...
        "isSpdxLicense()" should {
            "return true if the license is an SPDX license" {
                createPackageRule(packageWithoutLicense).apply {
                    val licenseRule = createLicenseRule(SpdxLicenseIdExpression("Apache-2.0"))
            ...
            "return false if the license is not an SPDX license" {
                createPackageRule(packageWithoutLicense).apply {
                    val licenseRule = createLicenseRule(SpdxLicenseIdExpression("invalid"))
                    val matcher = licenseRule.isSpdxLicense()
