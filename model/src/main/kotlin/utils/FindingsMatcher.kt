...
class FindingsMatcher(
    ...
    /**
     * Associate the given [copyrightFindings] to its corresponding applicable root licenses. If not root license is
     * applicable to a given copyright finding it is not contained in the result.
     */
    fun matchWithRootLicenses(
        licenseFindings: Set<LicenseFinding>,
        copyrightFindings: Set<CopyrightFinding>
    ): Map<LicenseFinding, Set<CopyrightFinding>> {
        fun TextLocation.directory(): String = path.substringBeforeLast(delimiter = "/", missingDelimiterValue = "")
